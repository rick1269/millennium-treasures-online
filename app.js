...
function renderAction(){const s=state,m=s.me,h=s.host===m?.id,a=s.auction,t=s.trade;
 ...
 if(s.phase==='select')return h?`<h2>从 ${s.lotCount} 张卡背中选心仪卡</h2><p>卡牌仍背面朝上。请点选一张心仪卡；选中后移至首位拍卖，拍下它时实付八折。选定首拍模式后，先进入拍前出售窗口。</p>${roundCardBacks(s,{selectable:true})}<p class="selection-help" role="status">${selectedPosition===null?'请先点选一张卡背。':`已选择第 ${selectedPosition+1} 张。`}</p><label>第一张模式<select id="firstMode"><option value="open" ${selectedFirstMode==='open'?'selected':''}>明拍</option><option value="sealed" ${selectedFirstMode==='sealed'?'selected':''}>暗拍</option></select></label>${s.direction===null?`<label>轮换方向<select id="direction"><option value="1" ${selectedDirection===1?'selected':''}>顺时针</option><option value="-1" ${selectedDirection===-1?'selected':''}>逆时针</option></select></label>`:''}<button type="button" class="primary" data-act="choose" ${selectedPosition===null?'disabled':''}>确认心仪卡，进入拍前出售</button>`:`<h2>等待起始玩家选心仪卡</h2><p>本轮抽出 ${s.lotCount} 张背面卡，由 ${esc(who(s.host))} 盲选一张，然后进入拍前出售和逐张拍卖。</p>${roundCardBacks(s)}`;
 ...
 if(s.phase==='pre-auction')return `<div class="action-rates"><span>拍卖行溢价</span>${[1,2,3,4].map(n=>`<span class="rate-chip">${n}星 ${Math.max(0,100+(s.rates[n]||0))}%</span>`).join('')}</div><h2>第 ${s.lotIndex+1} 张 · 拍前出售窗口</h2><p>本轮由 ${esc(who(s.host))}${s.players.find(p=>p.id===s.host)?.isBot?'（机器人自动）':''} 选定心仪卡并移至首拍。剩余 ${s.lotCount-s.lotIndex} 张背面卡，标有“下一张”的卡即将拍卖。玩家可在翻面前把手牌卖给拍卖行；每卖出一张，该星级议价减 10 个百分点。全员准备后由起始玩家翻牌。</p>${roundCardBacks(s)}${!s.ready?.[m.id]&&m.hand.length?`<label>我的藏品<select id="bankCard">${m.hand.map(c=>`<option value="${esc(c.id)}">${esc(c.name)} · 参考成交价 ${Math.floor(c.base*Math.max(0,100+(s.rates[c.star]||0))/100)} 金</option>`).join('')}</select></label>${button('卖给拍卖行','bankSell')}`:''}<div class="ready-status" role="status">我的状态：<strong>${s.ready?.[m.id]?'已准备':'未准备'}</strong> · ${Object.keys(s.ready||{}).length}/${s.players.length} 人已准备</div>${s.ready?.[m.id]?button('改为未准备','unready','ghost'):button('设为已准备','ready','primary')}${h&&s.players.every(p=>s.ready?.[p.id])?button('翻牌，开始拍卖','beginAuction','primary'):''}`;
 if(s.phase==='open')return `<h2>明拍 · ${esc(a.card.name)}${s.blindPosition===s.lotIndex?' · 心仪卡':''}</h2><div class="auction-preview">${card(a.card,s.blindPosition===s.lotIndex?'favorite-card':'')}<div><p>起拍价 <b>${Math.max(5,a.card.base/2)} 金</b></p><p>当前最高 <b>${a.highest?.amount??'暂无'} 金</b></p><p>加价至少 2 金；落槌后统一掷 1D6 修正。</p></div></div>${a.actor===m.id?`<div class="bid-shortcuts"><span>快速加价</span>${[2,5,10].map(step=>`<button type="button" class="ghost" data-bid-step="${step}">+${step}</button>`).join('')}</div><div class="controls"><input id="bid" type="number" inputmode="numeric" min="${a.highest?a.highest.amount+2:Math.max(5,a.card.base/2)}" max="${m.coins}" value="${a.highest?a.highest.amount+2:Math.max(5,a.card.base/2)}">${button('出价','bid','primary')}${button('退出','pass')}</div><div id="paymentPreview" class="payment-preview" aria-live="polite"></div>`:`<div class="callout">${a.bidders.includes(m.id)?`等待 ${esc(who(a.actor))} 行动。`:'你本场禁拍。'}</div>`}`;
...
main.addEventListener('click',async e=>{
 const shortcut=e.target.closest('[data-bid-step]');if(shortcut){const input=main.querySelector('#bid'),auction=state?.auction;if(input&&auction){const start=auction.highest?.amount??Math.max(5,auction.card.base/2);input.value=String(Math.max(Number(input.min),start+Number(shortcut.dataset.bidStep)));updatePaymentPreview();}return;}
...
main.addEventListener('input',e=>{if(e.target.matches('.auth-form [name="email"]'))authEmail=e.target.value;if(e.target.closest('.auth-form')&&authFeedback?.type==='error')setAuthFeedback(null);if(e.target.id==='firstMode')selectedFirstMode=e.target.value;if(e.target.id==='direction')selectedDirection=Number(e.target.value);if(['bid','sealedBid'].includes(e.target.id))updatePaymentPreview();if(['requestCard','freeCard','freeTarget','fromCoins','toCoins','responseCard'].includes(e.target.id))updateTradePreview();});
