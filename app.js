...
function cardBack(index,{selectable=false,selected=false,blind=false,current=false}={}){
 ...
 return selectable?`<button type="button" class="blind-card ${selected?'active':''}" data-position="${index-1}" aria-label="选择第 ${index} 张背面卡作为心仪卡" aria-pressed="${selected}">${contents}</button>`:`<div class="blind-card ${current?'current':''}" role="img" aria-label="第 ${index} 张背面卡${blind?'，心仪卡':''}${current?'，下一张拍卖':''}">${contents}</div>`;
}
...
function renderAction(){const s=state,m=s.me,h=s.host===m?.id,a=s.auction,t=s.trade;
 ...
 if(s.phase==='pre-auction')return `<h2>第 ${s.lotIndex+1} 张 · 拍前出售窗口</h2><p>本轮由 ${esc(who(s.host))}${s.players.find(p=>p.id===s.host)?.isBot?'（机器人自动）':''} 选定心仪卡。剩余 ${s.lotCount-s.lotIndex} 张背面卡，标有“下一张”的卡即将拍卖。玩家可在翻面前把手牌卖给拍卖行；每卖出一张，该星级议价减 10 个百分点。全员确认后由起始玩家翻牌。</p>${roundCardBacks(s)}${!s.ready?.[m.id]&&m.hand.length?`<label>我的藏品<select id="bankCard">${m.hand.map(c=>`<option value="${esc(c.id)}">${esc(c.name)} · 参考成交价 ${Math.floor(c.base*Math.max(0,100+(s.rates[c.star]||0))/100)} 金</option>`).join('')}</select></label>${button('卖给拍卖行','bankSell')}`:''}${!s.ready?.[m.id]?button('本张卡已结束出售','ready','primary'):`<div class="callout">已确认，等待其他玩家。${Object.keys(s.ready||{}).length}/${s.players.length} 人已确认。</div>`}${h&&s.players.every(p=>s.ready?.[p.id])?button('翻牌，开始拍卖','beginAuction','primary'):''}`;
