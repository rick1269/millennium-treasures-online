...
function seatMarkup(s,i){
 ...
 if(p)return `<div class="table-seat seat-${i} ${p.id===m?.id?'is-me':''} ${p.id===s.host&&s.phase!=='lobby'?'is-host':''}"><div class="seat-avatar" aria-hidden="true">${esc(p.avatar||'🦊')}</div><div class="seat-detail"><strong>${esc(p.name)}</strong><small>${p.isBot?'机器人':p.autoPilot?'托管中':p.isGuest?'游客':'玩家'} · ${p.coins} 金${s.phase!=='lobby'?` · ${p.count} 张`:''}</small></div>${owner&&lobby&&p.isBot?`<button type="button" class="seat-mini" data-act="seatBot" data-seat="${i}">移走</button>`:''}</div>`;
